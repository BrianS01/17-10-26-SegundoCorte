...
;(function ($)
{
    ...
    $.fn.TTSlider.slideShow = function (sequence_mode, index_next)
    {
        ...
        {
            case 'Fade':
                $.fn.TTSlider.effectFade(currentSlide, next);break;
            case 'Circlereveal':
                $.fn.TTSlider.effectCirclereveal(currentSlide, next);break;
            case 'RadialBlur':
                $.fn.TTSlider.effectRadialblur(currentSlide, next);break;
            case 'Wipe':
                $.fn.TTSlider.effectWipe(currentSlide, next);break;
            case 'SlideLeft':
                $.fn.TTSlider.effectSlideLeft(currentSlide, next);break;
            case 'SlideRight':
                $.fn.TTSlider.effectSlideRight(currentSlide, next);break;
            case 'SlideTop':
                $.fn.TTSlider.effectSlideTop(currentSlide, next);break;
            case 'SlideBottom':
                $.fn.TTSlider.effectSlideBottom(currentSlide, next);break;
            case 'Blind':
                $.fn.TTSlider.effectBlind(currentSlide, next);break;
            case 'Pixelate':
                $.fn.TTSlider.effectPixelate(currentSlide, next);break;
            case 'SimpleSlide':
                $.fn.TTSlider.simpleSlide(next);break;
            default:
                $.fn.TTSlider.effectNone(currentSlide, next);break;
        }
    ...
    function pixelate(v)
    {
        var size = v * 0.02, w = mycanvas.width * size, h = mycanvas.height * size;
    ...
    $.fn.TTSlider.effectForeground = function (div1)
    {
        ...
        for (var i = 0; i < length; i++)
        {
            ...
            {
                l = document.getElementsByClassName(div);
                style = window.getComputedStyle(l[0]), left = style.getPropertyValue("left");
                right = $("." + div).css("right");
            }
